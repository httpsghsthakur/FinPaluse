...
export const ForecastPage: React.FC = () => {
  ...
  const currentLiquidBalance =
    points.find((p) => p.isActual)?.actualBalance || 43270;
  ...
  const endForecastPoint = points[points.length - 1] || {
    forecastedBalance: 0,
  };
  const netDelta =
    (endForecastPoint.forecastedBalance || 0) - currentLiquidBalance;
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-0.5">
            90-day predictive balance model with automated recurring bill
            detection
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-slate-900/40 backdrop-blur-md border border-slate-800/60 rounded-[28px] p-5.5 space-y-1 shadow-[0_10px_30px_rgba(0,0,0,0.2)]">
          ...
          <div className="text-2xl lg:text-3xl font-bold font-mono text-white">
            {formatCurrency(currentLiquidBalance)}
          </div>
        ...
        <div className="bg-slate-900/40 backdrop-blur-md border border-slate-800/60 rounded-[28px] p-5.5 space-y-1 shadow-[0_10px_30px_rgba(0,0,0,0.2)]">
          ...
          <p className="text-xs text-slate-400">
            Occurs on{" "}
            {minForecastPoint.date
              ? formatDate(minForecastPoint.date, "MMM d, yyyy")
              : "N/A"}{" "}
            (Safe Buffer)
          </p>
        </div>
          ...
          actions={
            <div className="flex items-center gap-4 text-xs font-mono">
              ...
              <span className="flex items-center gap-1.5 text-slate-400">
                <span className="w-2.5 h-2 bg-indigo-500/20 rounded" />{" "}
                Confidence Band
              </span>
            </div>
          }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-slate-900/40 backdrop-blur-md border border-slate-800/60 rounded-[28px] p-6 space-y-4 shadow-[0_10px_30px_rgba(0,0,0,0.2)]">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-slate-400">
                Identified cash inflections in the next {range} days
              </p>
            </div>
            ...
            {events.map((ev) => {
              ...
              return (
                ...
                >
                  ...
                  <div
                    className={`font-mono font-bold ${isIncome ? "text-emerald-400" : "text-slate-200"}`}
                  >
                    {isIncome
        ...
        <div className="lg:col-span-1 bg-slate-900/40 backdrop-blur-md border border-slate-800/60 rounded-[28px] p-6 space-y-4 shadow-[0_10px_30px_rgba(0,0,0,0.2)]">
          ...
          <p className="text-xs text-slate-300 leading-relaxed">
            Finpluse’s predictive engine evaluates 180 days of historical
            cash-flow data. It factors in:
          </p>
          ...
          <ul className="text-xs text-slate-400 space-y-2 leading-relaxed">
            <li className="flex items-start gap-2">
              ...
              <span>
                Bi-weekly tech engineering payroll direct deposits on the 1st
                and 15th.
              </span>
            </li>
            <li className="flex items-start gap-2">
              ...
              <span>
                Fixed apartment rent lease of{" "}
                {formatCurrency(25000, profile.currency)} debited on the 1st of
                every month.
              </span>
            </li>
            <li className="flex items-start gap-2">
              ...
              <span>
                High-Yield Savings compound interest yield at 4.75% APY.
              </span>
            </li>
          </ul>

          <div className="p-3 rounded-2xl bg-slate-800/40 backdrop-blur-md border border-slate-700/50 text-[11px] text-slate-400">
            Confidence bounds widen organically over time to reflect
            discretionary dining and retail variance.
          </div>
        </div>
      </div>
    </div>
