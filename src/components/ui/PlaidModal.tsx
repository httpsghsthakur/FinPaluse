...
import { Building2, Lock, CheckCircle2, ShieldCheck, ArrowRight, Loader2 } from 'lucide-react';
...
const POPULAR_INSTITUTIONS = [
  ...
  { name: 'Marcus by Goldman Sachs', color: '#10B981', type: 'savings', mask: '9034' },
  { name: 'American Express', color: '#F59E0B', type: 'credit', mask: '1004' },
  { name: 'Capital One', color: '#6366F1', type: 'credit', mask: '5520' },
  { name: 'Fidelity Investments', color: '#059669', type: 'investment', mask: '3349' },
  { name: 'Robinhood', color: '#22C55E', type: 'investment', mask: '7721' },
];

export const PlaidModal: React.FC<{ onAccountAdded?: () => void }> = ({ onAccountAdded }) => {
  const { isPlaidModalOpen, closePlaidModal, showToast } = useUIStore();
  const [selectedInst, setSelectedInst] = useState<typeof POPULAR_INSTITUTIONS[0] | null>(null);
  ...
  const handleConnect = async (e: React.FormEvent) => {
    ...
    try {
      await api.connectAccount({
        ...
        balance: selectedInst.type === 'credit' ? -850.40 : selectedInst.type === 'savings' ? 12400.0 : 4520.0,
        currency: 'USD',
    ...
    >
      {step === 'select' && (
        <div className="space-y-4">
          <div className="flex items-center gap-2 p-3 bg-slate-900/80 border border-slate-800 rounded-xl text-xs text-slate-300">
            ...
            <span>Finpluse uses end-to-end read-only tokens. Your credentials are never stored.</span>
          </div>
            ...
            {POPULAR_INSTITUTIONS.map((inst) => (
              ...
              >
                ...
                <div className="truncate">
                  ...
                  <div className="text-[10px] text-slate-400 capitalize">{inst.type}</div>
                </div>
              </button>
            ))}
          </div>
      ...
      {step === 'credentials' && selectedInst && (
        <form onSubmit={handleConnect} className="space-y-4">
          <div className="flex items-center gap-3 p-3 bg-slate-900/60 border border-slate-800 rounded-xl">
            ...
            <div>
              <div className="text-sm font-semibold text-slate-200">{selectedInst.name}</div>
              <div className="text-[11px] text-slate-400">Direct OAuth Verification</div>
            </div>
          </div>

          <div className="space-y-3">
            <div>
              <label className="block text-xs font-medium text-slate-300 mb-1">User ID / Username</label>
              <input
                type="text"
                value={username}
      ...
      {step === 'connecting' && (
        <div className="py-8 flex flex-col items-center justify-center text-center space-y-3">
          ...
          <div className="text-sm font-semibold text-slate-200">Verifying secure OAuth credentials...</div>
      ...
      {step === 'success' && (
        <div className="py-8 flex flex-col items-center justify-center text-center space-y-3">
          ...
          <div className="text-sm font-bold text-slate-100">Account Linked Successfully!</div>
          <div className="text-xs text-slate-400">Importing recent transactions and balances...</div>
        </div>
      )}
    </Modal>
  );
