...
export const CitationChip: React.FC<CitationChipProps> = ({ groundedData, className }) => {
        ...
        <div className="mt-2 p-3 bg-slate-900/90 border border-slate-800 rounded-xl space-y-1.5 shadow-xl max-w-sm">
          ...
          <div className="grid grid-cols-2 gap-2 text-xs">
            {groundedData.map((item, idx) => (
              <div key={idx} className="bg-slate-800/50 p-1.5 rounded-lg border border-slate-700/50">
                <div className="text-[10px] text-slate-400">{item.label}</div>
                <div className="font-semibold text-slate-200 truncate">{item.value}</div>
              </div>
            ))}
          </div>
        </div>
